...
export function ViewProfile() {
  ...
  const user_logged: boolean =
    (window.localStorage.getItem("sessionId") as string) === username_url;
  ...
  useEffect(() => {
    fetch(
      ...
      }/linkhub/user/public?name=${username_url}`
    )
      .then((res) => {
        if (!res.ok) {
            ...
            <>
              ...
              <div className="mt-10 flex justify-center	">
                {user_logged ? (
                  <Link to={"/edit"}>
                    Edit this page{" "}
                    <img className="inline w-5" src={logo} alt="logo" />
                  </Link>
                ) : (
                  <Link to={"/"}>
                    Powered by Linkhub{" "}
...
function ProfileLink(props: { text: string; target: string; color: string }) {
  return (
    <a
      className={`p-4 bg-${props.color}-500 text-white hover:bg-${props.color}-400 rounded-full m-2 w-full text-center select-none`}
      href={props.target}
      target="_blank"
    >
      {props.text}
...
function UserNotFound() {
  return (
    <div className="flex flex-col items-center m-auto mt-20 p-5 max-w-lg bg-white rounded drop-shadow-xl">
      ...
      <Link
        to={"/"}
        className="p-4 bg-blue-500 text-white hover:bg-blue-400 rounded-full m-2 w-full text-center select-none"
      >
        Back to Linkhub
      </Link>
    </div>
