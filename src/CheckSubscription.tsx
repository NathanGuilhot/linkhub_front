export function CheckSubscription(pUserId:string, pSubEnd: number, pCallBack: (_v: boolean) => void) {
  ...
  if (pSubEnd !== undefined && pSubEnd !== 0 && pSubEnd > TimeInSecond) {
    ...
    fetch(`http://localhost:3003/pay/subscription/${pUserId}`).then((res: any) => {
      ...
      res.json().then((data: any) => {
        if (data.sub_end !== undefined && data.sub_end !== 0 && data.sub_end > TimeInSecond) {
          pCallBack(true);
        }
      }
      );
