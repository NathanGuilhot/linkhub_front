  ...
  return (<div className="bg-lime-900">
    ...
    <div className="p-5 bg-pink-200 w-full min-h-screen block lg:flex">
      <div className="flex-2">
        ...
        <p className="max-w-xl mt-5">The whole UI of the application has been developed with state-of-the-art tools such as Vite for the live server, React for the component based UI, React Router Dom to handle the application routing, and Tailwind for the CSS.</p>
        ...
        <Link className="bg-purple-900 p-5 mt-20 mb-10 rounded-full text-white cursor-pointer select-none" to={"/register"}>Get started for free</Link>
      </div>
      <div className="flex-1 p-5 m-5 mt-10"><img alt="Demo picture of the Linkhub UI" src={PictureDemo01} className="rounded-xl w-100"/></div>
    </div>
    <div className="p-5 bg-red-900 w-full min-h-screen">
      ...
      <p className="max-w-xl mt-5 text-pink-200">API Security was one of the major learning goal for this project.</p>
	    <p className="max-w-xl mt-5 text-pink-200">This manifests itself in multiple ways, such as obviously encrypting the password in the database, and verifying all API routes with a JSON Web Token (JWT).</p>
	    <p className="max-w-xl mt-5 text-pink-200">JWT encryption allows each user to have a token that cannot be tampered with, and thus allowing us to authenticate the user for each of the sensitive API calls in a simple way.</p>
      ...
      <Link className="bg-pink-200 p-5 mt-20 mb-10 rounded-full text-black cursor-pointer select-none" to={"/register"}>Get started for free</Link>
    </div>
    <div className="p-5 bg-pink-200 w-full min-h-screen">
      <h1 className="max-w-xl text-5xl text-purple-900 font-bold mt-10 main_font">Stripe API for subscriptions</h1>
      <p className="max-w-xl mt-5">Integrating the payment API Stripe was also a major goal for this project.</p>
      <p className="max-w-xl mt-5">I incorporated a subscription by creating a Stripe Costumer profile for each new person joining the website. I then only store the end date of the current subscription period, this way we only have to call the Stripe API again once this period is over, reducing latency and data usage.</p>
    ...
    <div className="p-5 pb-10 bg-red-900 w-full min-h-screen">
      ...
      <p className="max-w-xl mt-5 text-pink-200">In addition to the CRUD functionalities, I also implemented the ability to upload a custom profile picture to the server.</p>
      <p className="max-w-xl mt-5 text-pink-200">Any kind of user-facing interactions with the server represents a major security concern; that's why <b>the raw file is never sent to the server</b>: instead, it goes through a rescale function that uses the browser canvas API. The image is rescaled, the raw pixels are captured and compressed, and only then sent to the server and saved on disk.</p>
      <p className="max-w-xl mt-5 text-pink-200">The upload route obviously requires a valid JWT, preventing a malicious user from affecting other accounts.</p>
...
function LandingPageForm(){
  ...
  return (
    <>
    <form className="mt-3">
      <input className="rounded-md h-full p-4 text-lg w-full mb-5 md:w-fit" type="text" placeholder="linkhub.com/yourname" pattern="\w{2,16}" onChange={(e)=>{
